...
<style>
  ...
  background: url('{{ $watermark_image ?? 'images/image.png' }}') no-repeat center center;
...
<!-- ========================= PAGE 1 ========================= -->
<section class="page">
  <img class="header-image" src="{{ $header_image ?? 'images/yayedia.png' }}" alt="En-tête YAYE DIA BTP">
  ...
  <div class="parties">
    <div class="kv">
      <div><b>Monsieur :</b></div> <div>{{ $client->full_name ?? $contract->client->full_name ?? 'OMAR BADJI' }}</div>
      <div><b>Date et lieu de naissance :</b></div> <div>{{ $client->birth_date ?? $contract->client->birth_date ?? '27 décembre 1980' }} à {{ $client->birth_place ?? $contract->client->birth_place ?? 'BIGNONA' }}</div>
      <div><b>Adresse Personnelle :</b></div> <div>{{ $client->address ?? $contract->client->address ?? 'THIAROYE AZUR' }}</div>
      <div><b>Pays de Résidence :</b></div> <div>{{ $client->country ?? $contract->client->country ?? 'Sénégal' }}</div>
      <div><b>Nationalité :</b></div> <div>{{ $client->nationality ?? $contract->client->nationality ?? 'Sénégalais' }}</div>
      <div><b>Type et N° de pièce d'identité :</b></div> 
      <div>{{ $client->id_type ?? $contract->client->id_type ?? 'C.I.N' }} N° {{ $client->id_number ?? $contract->client->id_number ?? '1 001 1992 04680' }} délivrée le : {{ $client->id_issue_date ?? $contract->client->id_issue_date ?? '18/02/2017' }}</div>
      <div><b>Numéro mobile :</b></div> <div>{{ $client->phone ?? $contract->client->phone ?? '+221 77 618 41 19' }}</div>
    </div>
  ...
  <div class="bottom-refs tiny">
    <img src="{{ $footer_image ?? 'images/footer-image.png' }}" alt="Bannière footer">
...
qui définit les termes et conditions de leurs engagements respectifs.
  ...
  <p>
    L'assiette de l'ensemble immobilier est constituée par le terrain situé à {{ $contract->site->name ?? 'LELO SERERE' }}, d'une
    superficie totale de {{ $contract->site->total_area ?? '324ha 69a 80ca' }}, suivant la délibération N°002 /AKM.
  </p>

  <p class="article">Article 3 : Réservation</p>
  <p>
   Le présent contrat est conclu en vue de l'acquisition par le Client du terrain faisant l'objet du lot N°{{ $contract->lot->reference ?? '124/126' }}
   d'une superficie de {{ $contract->lot->area ?? '225m2' }} dans le cadre de l'ensemble immobilier visé à l'article 2.
  ...
  <p>
  4.1 Prix de vente du terrain<br>
  Le prix de vente ferme et définitif du terrain est fixé à {{ number_format($contract->total_amount ?? 7000000, 0, ',', ' ') }} ({{ \Illuminate\Support\Str::ucfirst(\NumberFormatter::create('fr', \NumberFormatter::SPELLOUT)->format($contract->total_amount ?? 7000000)) }}) Francs CFA. Le prix
...
Le prix stipulé au présent contrat est payable sur une durée de {{ $contract->payment_duration_months ?? '24' }} mois à raison d'un acompte de {{ number_format(($contract->total_amount ?? 7000000) * 0.15, 0, ',', ' ') }} Francs CFA et d'une mensualité de {{ number_format($contract->monthly_payment ?? 245900, 0, ',', ' ') }} Francs CFA.
  </p>
...
<p class="indent">
  Le présent contrat prend effet à compter de la date de sa signature pour une durée de {{ $contract->payment_duration_months ?? 'vingt-quatre' }} ({{ $contract->payment_duration_months ?? '24' }})
  mois.
</p>
...
<p class="center small">
  Fait en deux (02) exemplaires originaux, À Dakar, le {{ $contract_date ?? now()->format('d/m/Y') }}
</p>
  ...
  <div class="signatures">
    ...
    <div class="sign-box">
      <p><b>Le Client</b></p>
      <p class="muted">{{ $client_name ?? $client->full_name ?? $contract->client->full_name ?? 'OMAR BADJI' }}</p>
    </div>
  </div>
