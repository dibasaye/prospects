...
<section class="page">
  ...
  <!-- Affichage du contenu personnalisé OU par défaut -->
  @if(isset($custom_content) && !empty($custom_content))
    {!! $custom_content !!}
  @else
    <!-- Contenu par défaut uniquement les informations du client -->
    <div class="parties">
        <div class="kv">
            ...
            <div>{{ $contract->client->birth_date ?? '___' }} à {{ $contract->client->birth_place ?? '___' }}</div>

            <div><b>Adresse Personnelle :</b></div> 
            <div>{{ $contract->client->address ?? '___' }}</div>

            <div><b>Pays de Résidence :</b></div> 
            <div>{{ $contract->client->country ?? '___' }}</div>

            <div><b>Nationalité :</b></div> 
            <div>{{ $contract->client->nationality ?? '___' }}</div>

            <div><b>Type et N° de pièce d'identité :</b></div> 
            <div>{{ $contract->client->id_type ?? '___' }} N° {{ $contract->client->id_number ?? '___' }} délivrée le : {{ $contract->client->id_issue_date ?? '___' }}</div>

            <div><b>Numéro mobile :</b></div> 
            <div>{{ $contract->client->phone ?? '___' }}</div>
        </div>
    </div>
  ...
  <p class="indent">
    ...
    En cas d'exercice de cette faculté de résolution Unilatérale, YAYE DIA BTP notifie la rupture du contrat par exploit d'huissier mentionnant son intention
